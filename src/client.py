...
class Client:
    ...
    def onText(self, inp):
        if inp.startswith('!'):
            ...
            self.multicastInGroup(self.selectedGroup,
                                  msg.MessageType.Application, inp.strip("\n"))
        ...
        if messageType == msg.MessageType.Bye:
            ...
            if message.getCounter() == sender.getCounterForGroup(group) + 1:
                ...
                while debuff is not None:
                    sender.incrementCounterForGroup(group)
                    self.deliverApplicationMessage(
                        group, sender.getUsername(), debuff.getContent())
                    debuff = sender.tryDebuffMessageForGroup(message, group)

                return True

            else:
                lg.debug('buffering message, counter is %s, sender has %s' % (
                    message.getCounter(), sender.getCounterForGroup(group)))
    ...
    def joinGroup(self, groupname):
        ...
        for peer in reply.getContent():
            ...
            """Send Hello message"""
            self.unicastInGroup(
                group, mem, msg.MessageType.Hello, '', group.getCounter())
...
if __name__ == '__main__':
    ...
    parser.add_argument('tracker_addr', help="the tracker's address in the \
                         form of host:port (e.g. 123.123.123.123:12345)")
    parser.add_argument(
        '-t', help='execute tests (emulation mode) - provide file', action='store')
    parser.add_argument('-s', help="use sequencer for total ordering",
