...
parser.add_argument(
    'test_num', help='1 for first test, 2 for second', type=int, action='store')
parser.add_argument('-t', help='use total ordering (sequencer)',
                    action='store_true', default=False)
parser.add_argument('-c', help='close all remote processes',
                    action='store_true', default=False)
...
# Tracker
args = '--seq_addr %s:%s ' % (sequencer_host,
                              sequencer_port) if TOTAL_ORDERING else ''
...
if TOTAL_ORDERING:
    ...
    openRemoteProcess('sequencer', sequencer_host,
                      ROOT_PATH + 'sequencer.py ' + args)
